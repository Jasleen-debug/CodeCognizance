...
const executeCpp = async (filepath) => { //D:\CodeCognizance\backend\helpers\codes\bc4dc755-716d-4083-88e9-a5a42cbf4e58.cpp
  const jobId = path.basename(filepath).split(".")[0] //"bc4dc755-716d-4083-88e9-a5a42cbf4e58"."cpp"
  const output_filename =  `${jobId}.exe` // .exe for windows and .out is for mac and linux - bc4dc755-716d-4083-88e9-a5a42cbf4e58.exe
  ...
  return new Promise((resolve, reject) => {
    exec(`g++ ${filepath} -o ${outPath} && cd ${outputPath} && .\\${output_filename}`, //.\\ => ./ mac
      (error, stdout, stderr) => {
        if (error) {
          reject({error, stderr})
        }
